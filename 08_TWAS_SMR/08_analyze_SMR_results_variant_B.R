...
if (length(smr_files) == 0) {
  stop(msg .smr msg，msg 06b_run_smr_Anxiety.sh)
}
...
all_smr <- lapply(smr_files, function(f) {
  ...
  if (grepl("^GTEx", bname)) {
    ...
    tissue    <- paste(parts[2:(length(parts)-1)], collapse = "_")
  } else {
    source_db <- "BrainMeta"
    dataset   <- tail(parts, 1)
...
cat(\n=== TRAIT_A x disease_anxietymsg ===\n)
if (nrow(plot_data) >= 2) {
  ibs_d <- plot_data[dataset == "TRAIT_A",     .(Gene, tissue, source, b_IBS=b_SMR, p_IBS=p_SMR)]
  anx_d <- plot_data[dataset == "disease_anxiety", .(Gene, tissue, source, b_ANX=b_SMR, p_ANX=p_SMR)]
  shared <- merge(ibs_d, anx_d, by=c("Gene","tissue","source"))
  if (nrow(shared) > 0) {
    shared[, direction := ifelse(sign(b_IBS)==sign(b_ANX), msg, msg（msg）)]
    setorder(shared, p_IBS)
    print(shared[, .(Gene, tissue, source,
                     b_IBS=round(b_IBS,3), b_ANX=round(b_ANX,3),
                     p_IBS=formatC(p_IBS,format="e",digits=1),
                     p_ANX=formatC(p_ANX,format="e",digits=1),
                     direction)], row.names=FALSE)
    fwrite(shared, file.path(OUT_DIR, "SMR_shared_IBS_Anxiety.txt"), sep="\t")
  } else {
    cat(  msg SMR msg×msg\n)
  }
}
...
if (nrow(plot_data) > 0) {
  ...
  color_map <- c("TRAIT_A"="#2166AC", "disease_anxiety"="#D6604D")
  ...
  p <- ggplot(plot_data, aes(x = b_SMR,
    ...
    labs(title   = TRAIT_A x disease_anxiety SMR msg（msg）,
